...
dir <- "/Users/gavin/genome/inopinata_RNAseq_1-22-20/MS_code_7-2023/data/"
...
resTC_iL4$neg.log.p.adj <- -(log10(resTC_iL4$padj))
...
resTC_iL4.om$is.sig <- as.factor(resTC_iL4.om$is.sig)
levels(resTC_iL4.om$is.sig)[levels(resTC_iL4.om$is.sig)=="FALSE"] <- "No"
levels(resTC_iL4.om$is.sig)[levels(resTC_iL4.om$is.sig)=="TRUE"] <- "Yes"

#get lists for enrichment analyses, top 10% of significant genes ordered in various ways


resTC_iL4$int.direction <- ifelse(resTC_iL4$int_term_x_neg_one >0, "positive","negative")
